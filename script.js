...
function beginDraw(){const old=getDraw();if(old)return showResult(old.card);openModal(`<div class="draw-v70"><p class="eyebrow">DAILY DRAW</p><h2>每日一抽</h2><p class="draw-v70-intro">靜下心，深呼吸，想一想今天最想理解的是什麼。</p><div class="draw-v70-scene"><span class="draw-v70-glow" aria-hidden="true"></span><img class="draw-v70-cardback" src="daily-draw-card-back.png?v=8.0.0" alt="Inneroot 塔羅牌背"></div><p class="draw-v70-note">這張牌不是答案，而是一個陪你聆聽內在的提醒。</p><button class="primary draw-v70-button" id="go">專注當下，開始抽牌 ✦</button></div>`);document.getElementById("go").onclick=()=>{const card=cards[Math.floor(Math.random()*cards.length)];localStorage.setItem("innerootDailyDraw",JSON.stringify({date:today(),card}));touchDailyEntry();showResult(card)}}
...
(function(){
  const el=document.getElementById("desktopTimeGreeting");
  if(!el)return;
  const hour=new Date().getHours();
  el.textContent=hour<12?"早晨好":hour<18?"下午好":"晚上好";
})();
